...
<h1>SCUI Vite UI Admin</h1>

</div>

## 介绍
本项目是fork的SCUI的1.6.6版，原版采用vue-cli脚手架开发实现，本项目将vue-cli替换为vite最新版，兼容处理了scui的所有组件。

本项目的终极目标是更改为组合式 API (Composition API),利用vue3的setup语法糖重构，并将vuex替换为pinia, TS暂时不考虑，会增加使用者的负担，有背SCUI作者的初衷！【已完成基本的组件兼容，语法修改还在进行中....】

SCUI是一个中后台前端解决方案，基于VUE3和elementPlus实现。
...
``` sh
# 克隆项目
git clone https://gitee.com/qbiancheng/scui-vite-ui.git
